...
$.fn.extend({
    animateCss: function(t, e) {
        ...
        return this.addClass("animated " + t).one(n,
        function() {
...
window.Aria = {
    init: function() {
        this.action.init(),
        THEME_CONFIG.ENABLE_PJAX && this.pjax(),
    ...
    doPjaxCompleteAction: function() {
        if ($("#header").removeClass("slideOutUp").addClass("slideInDown"), $("#body").removeClass("fadeOut").addClass("fadeIn"), THEME_CONFIG.ENABLE_FANCYBOX && this.fancybox(), THEME_CONFIG.SHOW_HITOKOTO && this.hitokoto(), THEME_CONFIG.ENABLE_LAZYLOAD && this.lazyload(), this.hljs.init(), this.commentPlus.init(), this.toc.init(), "undefined" != typeof _hmt && _hmt.push(["_trackPageview", location.pathname + location.search]), window._gaq && _gaq.push(["_trackPageview"]), window.ga && ga("send", "pageview", {
            page: location.pathname,
            title: document.title
        }), THEME_CONFIG.ENABLE_MATHJAX && "undefined" != typeof MathJax && MathJax.Hub.Queue(["Typeset", MathJax.Hub]), "function" == typeof loadMeting && loadMeting(), document.getElementsByClassName("dplayer").length) for (var t = dPlayerOptions.length,
        e = [], n = 0; n < t; n++) e.push(new DPlayer({
            ...
            lang: dPlayerOptions[n].lang,
            screenshot: dPlayerOptions[n].screenshot,
            hotkey: dPlayerOptions[n].hotkey,
            preload: dPlayerOptions[n].preload,
            logo: dPlayerOptions[n].logo,
            volume: dPlayerOptions[n].volume,
            mutex: dPlayerOptions[n].mutex,
    ...
    fancybox: function() { ($(".post-content img").length || $(".comment-content img").length) && ($("img:not([class~='link-avatar'],[no-fancybox])", ".post-content").wrap(function() {
            ...
            spinnerTpl: '<img style="position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);" src="' + THEME_CONFIG.THEME_URL + '/assets/img/loading.svg">'
        }))
    },
    hitokoto: function() {
        $.ajax({
    ...
    hljs: {
        init: function() {
            $("pre code").each(function(t, e) {
                ...
                var n = null == $(this).attr("class").match(/lang-(\w+)/) ? "CODE": $(this).attr("class").match(/lang-(\w+)/)[1].toUpperCase();
                $(this).attr("data-lang", n),
                $(this).after('<a class="copy-code" href="javascript:" data-clipboard-target="#hljs-' + t + '" title="拷贝代码"><i class="iconfont icon-aria-copy"></i></a>')
            }),
...
Aria.commentPlus = {
    ...
    ajaxComment: function() {
        ...
        $("#comment-form").submit(function() {
            ...
            return e.attr("disabled", !0).css("cursor", "not-allowed"),
            o.css({
                opacity: ".5"
            }),
            $("textarea", o).css({
                background: 'url("' + THEME_CONFIG.THEME_URL + '/assets/img/loading.svg") center center no-repeat'
            }),
            ...
            $.ajax({
                ...
                success: function(t) {
                    if (!$("#comments", t).length) {
                        var e = "error" === $("title").eq(0).text().trim().toLowerCase() ? $(".container", t).eq(0).text() : "评论提交失败！";
                        return s.alert(e),
                        r(!1),
                        !1
                    }
                    ...
                    $("input,textarea", o).attr("disabled", !1),
                    $("#textarea").val(""),
                    i = $(".comment-list", t).html().match(/id=\"?comment-\d+/g).join().match(/\d+/g).sort(function(t, e) {
                        return t - e
                    }).pop(),
                    "" === c ? ($(".comment-list").length ? $(".prev").length || (n = $("#li-comment-" + i, t), $(".comment-list").first().prepend(n.addClass("animated fadeInUp"))) : (n = $("#li-comment-" + i, t), $("#response").after('<div class="comment-data"><ol class="comment-list"></ol></div>'), $(".comment-list").first().prepend(n.addClass("animated fadeInUp"))), $("html,body").animate({
                        scrollTop: $("#response").offset().top - 100
                    },
                    1e3)) : (n = $("#li-comment-" + i, t), $("#" + c).find(".comment-children").length || $("#" + c).append('<div class="comment-children"><ol class="comment-list"></ol></div>'), $("#" + c + " .comment-children .comment-list").first().prepend(n.addClass("animated fadeInUp")), TypechoComment.cancelReply()),
                    a = parseInt($("#response").text()),
                    $("#response").html($("#response").html().replace(/\d+/, a + 1)),
                    r(!0),
                    s.confirm("评论提交成功！")
                },
                error: function(t) {
...
Aria.action = {
    ...
    gotop: function() {
        $(window).scroll(function() {
            if (100 < $(window).scrollTop() ? ($("#go-top").fadeIn(500), $("#site-avatar").css({
                height: "25px",
                width: "25px",
            ...
            function() {
                ...
                for (var a in e) {
                    var o = "#" + e[a];
                    $(o).offset().top > t + 100 || (n ? $(o).offset().top >= $(n).offset().top && (n = o) : n = o)
                }
                if (n) $("#toc a").removeClass("toc-active"),
...
Aria.toc.init = function() {
    if ($("#toc").length) {
        this.createDirectory(document.getElementsByClassName("post-content")[0], document.getElementById("toc"), !0);
        new SmoothScroll('#toc a[href*="#"]', {
            offset: 80
        });
...
Aria.toc.children = function(t, e) {
    var n, a, o, i = [],
    s = "object" == typeof e,
    r = "string" == typeof e;
    for (a = 0, o = t.length; a < o; a++) 1 !== (n = t[a]).nodeType && 9 !== n.nodeType || !(!e || s && e.test(n.tagName.toLowerCase()) || r && n.tagName.toLowerCase() === e) || i.push(n);
    return i
},
Aria.toc.createDirectory = function(t, e, n) {
    var a, o, i, s, r, c, l, d, h, m = [],
    p = [];
    for (a = function(t, e, n) {
        var a, o, i = Aria.toc.children(t.childNodes, /^h[2-3]$/),
        s = [],
        r = 1,
        c = 1,
        l = 0;
        for ((Math.random() + "").replace(/\D/, ""); i.length;) o = i.shift(),
        e.push(o.innerHTML),
        r < (a = +o.tagName.match(/\d/)[0]) ? (s.push(1), c += 1) : a === c || c < a && a <= r ? (s.push(0), c = c) : a < c && (s.push(a - c), c = a),
        l += s[s.length - 1],
        r = a,
        o.id = o.id || "toc-" + o.innerText,
        o.id = o.id.replace(/[\s|\~|`|\!|\@|\#|\$|\%|\^|\&|\*|\(|\)|\_|\+|\=|\||\|\[|\]|\{|\}|\;|\:|\"|\'|\,|\<|\.|\>|\/|\?]/g, ""),
        n.push(o.id);
        return 0 !== l && 1 === s[0] && (s[0] = 0),
        s
    } (t, m, p), s = o = document.createElement("ul"), dirNum = [0], d = 0, h = a.length; d < h; d++) {
        if (1 === (i = a[d])) r = document.createElement("ul"),
        s.lastElementChild || s.appendChild(document.createElement("li")),
        s.lastElementChild.appendChild(r),
        s = r,
        dirNum.push(0);
        else if (i < 0) for (i *= 2; i++;) i % 2 && dirNum.pop(),
        s = s.parentNode;
    ...
    s.event.special[i] = {
        ...
        add: function(t) {
            ...
            function e(t, e, n) {
                var a = s(this),
                o = s.data(this, c);
                o.w = void 0 !== e ? e: a.width(),
                o.h = void 0 !== n ? n: a.height(),
                i.apply(this, arguments)
            }
            if (s.isFunction(t)) return i = t,
            e;
            i = t.handler,
            t.handler = e
        }
